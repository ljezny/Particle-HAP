...
void generateAccessoryKey(ed25519_key *key) {
    ...
    hkLog.info("wc_ed25519_init key: r:%d\n",r);
    WC_RNG rng;
    r = wc_ed25519_make_key(&rng, ED25519_KEY_SIZE, key);
    hkLog.info("wc_ed25519_make_key key: r:%d\n",r);
...
void HKConnection::writeEncryptedData(uint8_t* payload,size_t size) {
    hkLog.info("BEGIN: writeEncryptedData");
    byte nonce[12];
    ...
    while (payload_offset < size) {
        ...
        if (r) {
            hkLog.info("Failed to chacha encrypt payload (code %d)\n", r);
            client.stop();
            return;
        }
        ...
        if(isConnected()){
            ...
            if (err != 0) {
              hkLog.trace("writeEncryptedData:: failed (error = %d), number of bytes written: %d", err, bytes);
            }
        }
    }
    ...
    hkLog.info("END: writeEncryptedData");
}

void HKConnection::decryptData(uint8_t* payload,size_t *size) {
    hkLog.info("BEGIN: decryptData");
    ...
    while (payload_offset < payload_size) {
        ...
        if (r) {
            hkLog.info("Failed to chacha decrypt payload (code %d)\n", r);
...
void HKConnection::writeData(uint8_t* responseBuffer,size_t responseLen) {
    hkLog.info("BEGIN: writeData");
    hkLog.info("writeData responseLen = %d\n", responseLen);
    if(isConnected()){
        if(isEncrypted) {
...
void HKConnection::handleConnection() {
    ...
    if (len > 0) {
        RGB.control(true);
        RGB.color(255, 255, 0);
        hkLog.info("Request Message read length: %d \n", len);
        ...
        if (!strcmp(msg.directory, "pair-setup")){
            ...
            hkLog.info("Handling Pair Varify...\n");
            ...
            hkLog.info("Handling message request: %s\n",msg.directory);
...
bool HKConnection::handlePairVerify(const char *buffer) {
    ...
    switch (state) {
        case State_Pair_Verify_M1: {
            ...
            hkLog.info("wc_curve25519_export_public_ex: r:%d\n",r);
            ...
            hkLog.info("crypto_curve25519_shared_secret: %d\n", r);
            ...
            hkLog.info("wc_ed25519_sign_msg: r:%d\n",r);
            ...
            hkLog.info("wc_ChaCha20Poly1305_Encrypt: r:%d\n",r);
        ...
        case State_Pair_Verify_M3: {
            ...
            hkLog.info("wc_ChaCha20Poly1305_Decrypt: r:%d\n",r);
            ...
            hkLog.info("wc_ed25519_verify_msg: r:%d\n",r);
            if(verified) {
                completed = true;
    ...
    *stateRecord.data = (char)state+1;
    switch (state) {
        ...
        case State_M3_SRPVerifyRequest: {
            ...
            hkLog.info("wc_SrpComputeKey: r:%d\n",r);
            r = wc_SrpVerifyPeersProof(&srp, (byte*) proofStr, proofLen);
            hkLog.info("wc_SrpVerifyPeersProof: r:%d\n",r);
            if (r != 0) { //failed
